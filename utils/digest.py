...
def _build_chokepoints_section(extractions: dict, mode: str = "cross") -> str:
    """
    Attempt to compute and format the chokepoints/entity section.
    Returns an empty string on failure — the rest of the digest still renders.

    mode="cross": entities appearing in 3+ filings (default, current behavior)
    mode="full":  all named entities from every filing, no threshold
    """
    try:
        from chokepoints import (compute_chokepoints, format_chokepoints_section,
                                 format_full_entities_section)
    except ImportError:
        return ""

    try:
        if mode == "full":
            cp = compute_chokepoints(extractions, min_mentions=1)
            total = sum(len(v) for v in cp.values())
            print(f"[digest] Full entity index: {total} entities "
                  f"({len(cp['vendors'])} vendor · "
                  f"{len(cp['customers'])} customer · "
                  f"{len(cp['competitors'])} competitor)")
            return format_full_entities_section(cp, n_filings=len(extractions))
        else:
...
def build_digest(extractions: dict, ranking: dict, *,
                 ...
                 chokepoints_mode: str = "cross") -> str:
    """
    ...
    chokepoints_md = _build_chokepoints_section(extractions, mode=chokepoints_mode)
    if chokepoints_md:
