...
_GENERIC_PHRASES: set[str] = {
    ...
    # Sentence starters that look like proper nouns
    "some", "certain", "limited", "multiple", "additional", "both",
    "commercial", "definitive", "ten-year", "ten", "per", "each",
    "international", "domestic", "federal", "national", "global",
    "annual", "quarterly", "fiscal", "current", "former", "new",
    "partnerships", "partnership", "agreements", "agreement", "services",
    "products", "solutions", "systems", "operations", "activities",
}
...
def format_full_entities_section(chokepoints: dict, n_filings: int) -> str:
    """
    Render all named entities (no min threshold) as a full entity index.
    Used with --chokepoints full mode. Shows every vendor, customer, and
    competitor named in any filing, sorted alphabetically within each category.
    Entities named by multiple filers show multiple tickers naturally.
    """
    lines = [
        "## Entity index for this run",
        "",
        f"_All named vendors, customers, and competitors extracted from "
        f"{n_filings} filing{'s' if n_filings != 1 else ''}. "
        f"Entities appearing in multiple filings indicate shared relationships._",
        "",
    ]
    ...
    for category in ("vendors", "customers", "competitors"):
        ...
        for e in sorted(entries, key=lambda x: x["name"].lower()):
            ticker_list = " · ".join(e["tickers"])
            lines.append(f"- {e['name']} — {ticker_list}")
        lines.append("")

    if not any_results:
        lines.append("_No named entities found in the current corpus._")
        lines.append("")
