...
class FilingResult:
    ...
    operating_margin_pct: float = 0.0   # from financial_performance extraction, 0 if missing
    revenue_growth_pct: float = 0.0     # from financial_performance extraction, 0 if missing
...
def _evaluate_filing(ticker: str, ext: dict, manifest_lookup: dict) -> FilingResult:
    ...
    # Pull financial performance scores for sorting. Defaults to 0 if the
    # financial_performance category hasn't been extracted yet for this filing.
    fp = ext.get("financial_performance", {})
    operating_margin = float(fp.get("operating_margin_pct") or 0.0)
    revenue_growth = float(fp.get("revenue_growth_pct") or 0.0)
...
def _make_sort_key(mode: str = "ctmg"):
    ...
    def key(r: FilingResult):
        ...
        t = -len(r.triggers)
        m = -(r.operating_margin_pct or 0.0)
        g = -(r.revenue_growth_pct or 0.0)
        if mode == "ctmg":
...
def _print_results(results: list[FilingResult], mode: str = "ctmg") -> None:
    ...
    for i, r in enumerate(results, 1):
        ...
        margin_note = f"  [margin={r.operating_margin_pct:.1f}%]" if r.operating_margin_pct else ""
        growth_note = f"  [growth={r.revenue_growth_pct:.1f}%]" if r.revenue_growth_pct else ""
