...
CATEGORY_CONFIG = {
    ...
    "financial_performance":  (FinancialPerformanceSchema,   ["item_7", "item_8"]),
}
...
- "free_cash_flow_usd": Free cash flow in full USD. First look in MD&A (item_7) for an explicitly labeled FCF figure (e.g., "free cash flow was $X"). If not explicitly labeled, look in the cash flow statement (item_8) for "net cash provided by operating activities" and the capital expenditure line (typically "purchases of property and equipment" or "capital expenditures"), then calculate: FCF = operating cash flow minus capex. Use consolidated figures only. null if neither operating cash flow nor capex figures are available.
