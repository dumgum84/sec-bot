...
SUMMARIES_PATH = OUTPUT_DIR / "digest_summaries.json"
RUN_MANIFEST_PATH = OUTPUT_DIR / "run_manifest.json"
WATCH_QUEUE_PATH = OUTPUT_DIR / "watch_queue.json"
DEFAULT_DIGEST_PATH = OUTPUT_DIR / "digest.md"

# Load .env from the script's directory for the API key
...
You will be given the structured facts our extractor pulled from the filing, the tier the bot assigned, and the triggers that fired.
...
BUSINESS — what the company does in plain language. Cover the segments (with brief descriptions if present), primary geographies, and how the company positions itself competitively.

WHY THIS FILING MATTERS — synthesize the events, governance changes, impairments, litigation, and regulatory matters into prose. Connect related facts where the data supports it. Open with the substantive fact, not a label about the filing's importance. Do NOT use the words "critical", "notable", "quiet", or paraphrases like "warrants attention", "of note", "deserves scrutiny" — the tier is already shown in the header. Skip this paragraph if nothing substantive fired.

STRUCTURAL POSITION — surface concrete details from the data that shape how this business actually operates and what risks it carries. Draw from customer/vendor concentrations, self-disclosed disadvantages, and named competitors. Surface the items that are distinctive to this company; skip generic ones (every company faces "competition" and "regulatory risk", so don't bother repeating those). Use whatever percentages, named parties, or descriptors the extraction provides verbatim — do not invent figures. Name competitors specifically when they're recognizable or distinctive; if the list is long, indicate the breadth and name the most prominent. Skip this paragraph entirely if the relevant fields have nothing substantive.

FORWARD — capex plans and named initiatives. Brief.
...
- Roughly 250-400 words. Adjust length to match how much substance exists.
...
def _build_summary_prompt(ticker: str, ext: dict, ranking_entry: dict,
    ...
    tier = ranking_entry.get("tier", "quiet")
...
def _cache_key(ticker: str, accession: str) -> str:
    """A summary is keyed by (ticker, accession) so the cache survives re-ranks."""
...
def build_digest(extractions: dict, ranking: dict, *,
    ...
    lines = [
        ...
        f"{summary.get('notable_count', 0)} notable · "
        f"{summary.get('quiet_count', 0)} quiet.",
        "",
        f"_Ranking rules: {ranking.get('rules_version', '?')}. "
        f"Summaries: {SUMMARY_MODEL if use_llm else 'template-only'}. "
        f"Auto-generated by digest.py — safe to overwrite._",
        "",
        "---",
        "",
    ]
    ...
    for i, fs in enumerate(filings, 1):
        ...
        if ext is None:
            lines.append(f"### {ticker} [{fs.get('tier', 'quiet').capitalize()}]")
            lines.append("_(extraction missing — re-run extract.py)_")
            lines.append("")
            continue

        key = _cache_key(ticker, accession)
